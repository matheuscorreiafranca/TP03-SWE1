...
Este documento descreve como fazer o deploy da aplicação TP02-CBTSWE1 no Apache Tomcat 10+.
...
```bash
cd /home/dev/sistema/sistema/TP02-CBTSWE1
...
```bash
cd TP02-CBTSWE1/src/main/java
javac -cp /usr/share/tomcat10/lib/servlet-api.jar -d /tmp/classes -encoding UTF-8 *.java
```
...
```bash
sudo cp deploy/tomcat/ROOT.xml /var/lib/tomcat10/conf/Catalina/localhost/TP02-CBTSWE1.xml
```

### 2. Copiar aplicação para webapps

```bash
sudo mkdir -p /var/lib/tomcat10/webapps/TP02-CBTSWE1/WEB-INF/classes
sudo cp -r src/main/webapp/* /var/lib/tomcat10/webapps/TP02-CBTSWE1/
sudo cp /tmp/classes/*.class /var/lib/tomcat10/webapps/TP02-CBTSWE1/WEB-INF/classes/
```

### 3. Ajustar permissões

```bash
sudo chown -R tomcat:tomcat /var/lib/tomcat10/webapps/TP02-CBTSWE1
```
...
A aplicação estará acessível em: `http://localhost:8080/TP02-CBTSWE1/`
...
- **Produtos:** http://192.168.1.72:8002/ViewServlet
- **Créditos:** http://192.168.1.72:8002/creditos
...
- GitHub do projeto: https://github.com/matheuscorreiafranca/TP02-CBTSWE1
